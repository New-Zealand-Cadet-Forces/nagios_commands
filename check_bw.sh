...
nagios_r()
{
	...
	echo "check_bw router=$hostname status=OK mode=$mode bandwidth_usage=$final percent=$perc" >> /var/log/nagios3/nagios.log
	exit 0;
	fi
	if [ `echo ${fcomp%.*}` -ge  `echo ${wcomp%.*}` ] && [ `echo ${fcomp%.*}` -lt `echo ${ccomp%.*}` ]
	then
	echo "Warning: Current $mode bandwidth usage is $final Mb/s, $perc% used"
	echo "check_bw router=$hostname status=Warning mode=$mode bandwidth_usage=$final percent=$perc" >> /var/log/nagios3/nagios.log
	exit 1;
	fi
	if [ `echo ${fcomp%.*}` -ge `echo ${ccomp%.*}` ]
	then 
	echo "Critical: Current $mode bandwidth usage is $final Mb/s, $perc% used"
	echo "check_bw router=$hostname status=Critical mode=$mode bandwidth_usage=$final percent=$perc" >> /var/log/nagios3/nagios.log
	exit 2;
	fi
