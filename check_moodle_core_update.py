...
if not "Log In" in html:
    try:
        updateurl = 'https://' + host + '/admin/index.php?cache=1' if usehttps else 'http://' + host + '/admin/index.php?cache=1'
        r = br.open(updateurl)
    except mechanize.URLError, err:
    ...
    updatepage = r.read()
    if "There is a newer Moodle version available!" in updatepage:
        print "CRITICAL - Moodle core upgrade available!"
        sys.exit(STATE_CRITICAL)
    elif "Install available updates " in updatepage:
        print "WARNING - Moodle plugin upgrades available"
        sys.exit(STATE_WARNING)

    print "OK - No warnings about upgrades located"
    sys.exit(STATE_OK)
else:
